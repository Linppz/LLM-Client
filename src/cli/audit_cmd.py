...
def show_audit(
    last: Annotated[int, typer.Option(help="要显示最近多少条审计日志？")] = 5,
) -> None:
    ...
    for i in task.audit_logs[-last:]:
        table.add_row(
            i.template_name, i.version_hash, str(i.timestamp), str(i.variables)
        )
    console.print(table)
...
def show_difference(
    usertemplate: Annotated[str, typer.Argument(help="文件内容")],
    v1: Annotated[str, typer.Argument(help="文件1")],
    v2: Annotated[str, typer.Option(help="文件2")],
) -> None:
    engine = PromptTemplate(settings.LLM_DEFAULT_MODEL, cli_config.paths.template_dir)
    task = PromptRegistry(
        engine, cli_config.paths.manifest_path, cli_config.paths.audit_log_path
    ...
    if result is None:
        console.print("No such template or versions found.")
    else:
        console.print("Differences between versions:")
        for line in result:
            console.print(line)
